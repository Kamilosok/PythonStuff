...
class MyGame(arcade.Window):
    ...
    def add_coins(self):
        ...
        for i in range(COIN_COUNT_INCREMENT):
            ...
            coin.change_x = random.randrange(SPRITE_SIZE, SCREEN_WIDTH - SPRITE_SIZE)
            coin.change_y = random.randrange(SPRITE_SIZE, SCREEN_HEIGHT - SPRITE_SIZE)
    ...
    def update(self, delta_time):
        ...
        # odbijanie sie poza ekran
        for sprite in self.coin_list:

            if sprite.position[0] < 0 or sprite.position[0] > SCREEN_WIDTH:
                sprite.change_x *= -1
            if sprite.position[1] < 0 or sprite.position[1] > SCREEN_HEIGHT:
                sprite.change_y *= -1
        ...
        if total_program_time > self.last_fps_reading:
            ...
            if total_program_time > 5:
                ...
                if total_program_time % 2 == 1:
                    ...
                    arcade.draw_text(output, 300, 300, arcade.color.BLACK, 30)
                    print(output, end="")
                    if len(self.coin_list) >= STOP_COUNT or self.fps.get_fps() < 1:
                        pyglet.app.exit()
                        return
